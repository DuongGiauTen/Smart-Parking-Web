...
const VALID_CARD_IDS = ['USER_01', 'USER_02', 'USER_03', 'USER_04']
...
export default function GateEntry() {
  ...
  const processCardScan = (card) => {
    ...
    if (!isSystemOnline) {
      // AF2: Offline mode — barrier opens regardless
      setEntryMessage(
  ...
  useEffect(() => {
    const handleKeyDown = (e) => {
      ...
      if (e.altKey && e.key === '3') {
        ...
        setCurrentSession(null)
        setCurrentCardInfo(null)
        setEntryMessage('Mất kết nối hệ thống — chế độ Offline (AF2) kích hoạt')
      }
      if (e.altKey && e.key === '4') {
        // Alt+4: Giả lập quét thẻ ngẫu nhiên và chạy đúng luồng UC-02A
        e.preventDefault()
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 relative z-10 items-stretch">
        ...
        <div className="xl:col-span-7 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ['Lượt xe vào hôm nay', mockDB.getAccessLog().filter(l => l.action === 'ENTRY' && l.result === 'SUCCESS').length.toString(), 'directions_car'],
              ['Xe đang trong bãi', mockDB.getAllSessions().filter(s => s.status === 'active').length.toString(), 'hourglass_top'],
          ...
          <div className={`bg-white rounded-3xl p-6 border-2 transition-colors duration-300 shadow-md ${demoState === 'invalid' ? 'border-red-200 bg-red-50/30' : 'border-blue-100'}`}>
            ...
            <div className="relative">
              ...
              <input
                ...
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && scanCard.trim()) {
                    processCardScan(scanCard.trim())
                  }
                }}
            ...
            {demoState === 'invalid' && (
              <p className="mt-2 text-sm font-bold text-red-600 flex items-center gap-1">
                <span className="material-symbols-outlined text-sm">error</span> [EF1a] Thẻ bị khóa hoặc chưa được đăng ký trong hệ thống.
              </p>
            )}
            {demoState === 'full' && (
              <p className="mt-2 text-sm font-bold text-orange-600 flex items-center gap-1">
                <span className="material-symbols-outlined text-sm">warning</span> [EF2a] Bãi đã đầy — không thể cấp phép vào.
              </p>
            )}
            <p className="mt-3 text-xs text-slate-400">Phím tắt: Alt+4 quét ngẫu nhiên · Alt+1/2/3 demo lỗi · Esc đặt lại</p>
          </div>
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200/80 overflow-hidden flex-1 flex flex-col">
                ...
                <div>
                  ...
                  <p className="font-headline font-bold text-xl text-slate-800 leading-tight">
                    {demoState === 'invalid'
                      ...
                      : currentCardInfo?.name || 'Chờ quét thẻ...'}
                  </p>
                  {currentCardInfo && (
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50">
              <div className="grid grid-cols-2 gap-4 mb-5">
                <button
                  onClick={handleOpen}
                  disabled={demoState === 'full' || isBarrierOpen}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {accessLog.map((log) => {
                  const resultColor =
                    log.result === 'SUCCESS'
                      ...
                      : log.result.includes('INVALID') || log.result.includes('EF1a')
                ...
                {accessLog.length === 0 && (
                  <tr>
                    <td colSpan={5} className="p-8 text-center text-slate-400 text-sm">Chưa có lượt truy cập nào được ghi nhận</td>
                  </tr>
                )}
              </tbody>
            </table>
