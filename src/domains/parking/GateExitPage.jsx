...
export default function GateExit() {
  ...
  // Guest payment states
  const [isWaitingPayment, setIsWaitingPayment] = useState(false)
  const [pendingExitData, setPendingExitData] = useState(null) // { session, fee, cardInfo }
  ...
  const handleExitScan = () => {
    ...
    if (!isGuest) {
      // Member (Sinh viên / Cán bộ): Ghi nợ tự động → đóng session → mở Barrier ngay
      ...
      setStatusMessage(
        `Xe ${session.vehicle} đã ra. Phí ${fee.toLocaleString('vi-VN')} VNĐ đã ghi nợ tự động.`
      )
      setExitLog(mockDB.getAccessLog(10).filter(l => l.action === 'EXIT'))
    } else {
      // Guest (Khách vãng lai): Chờ thanh toán — Barrier KHÔNG mở cho đến khi có [OK]
      setScanResult({ ...session, fee })
      setPendingExitData({ session, fee, cardInfo })
      setIsWaitingPayment(true)
      setStatusMessage(
        `Khách vãng lai · Xe ${session.vehicle} · Phí: ${fee.toLocaleString('vi-VN')} VNĐ — Đang chờ thanh toán.`
      )
    }
  ...
  // UC-02B Guest: Callback [OK] — thanh toán thành công → đóng session → mở Barrier
  ...
  return (
    <div className="p-6 md:p-8 space-y-8 bg-slate-50 min-h-full font-inter relative">
                ...
                <div className="hidden sm:block">
                  ...
                  {demoState !== 'offline' ? (
                    ...
                    <p className="text-sm font-bold text-slate-400 flex items-center gap-1">
                      <span className="material-symbols-outlined text-sm">wifi_off</span> Mất tín hiệu
                    </p>
                  )}
                </div>
              </div>
          ...
          <div className={`bg-white rounded-3xl p-6 border-2 transition-colors duration-300 shadow-md ${isWaitingPayment ? 'border-amber-300 bg-amber-50/30' : demoState === 'normal' ? 'border-orange-100 bg-orange-50/20' : 'border-blue-100'}`}>
            ...
            <div className="relative">
              ...
              <input
                autoFocus
                type="text"
                placeholder="Nhập mã thẻ hoặc quét NFC..."
                value={exitInput}
                onChange={(e) => setExitInput(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === 'Enter' && exitInput.trim()) handleExitScan()
                }}
                className={`w-full pl-14 pr-4 py-4 rounded-xl text-lg font-bold font-mono outline-none transition-all border ${isWaitingPayment ? 'bg-amber-50 border-amber-300 text-amber-900' : demoState === 'normal' ? 'bg-orange-50/50 border-orange-200 focus:border-orange-400 text-orange-900' : 'bg-slate-50 text-blue-900 border-slate-200 focus:border-blue-400 focus:bg-white focus:shadow-[0_0_0_4px_rgba(59,130,246,0.1)]'}`}
              />
            </div>
            <div className="mt-4 flex flex-col sm:flex-row gap-3">
              <button
                onClick={handleExitScan}
                disabled={isProcessing || !exitInput.trim() || isWaitingPayment}
                className={`w-full sm:w-auto py-3 px-5 rounded-xl font-bold text-white transition ${isProcessing || !exitInput.trim() || isWaitingPayment ? 'bg-slate-300 text-slate-600 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}
              >
                {isProcessing ? 'Đang xử lý...' : 'Xác nhận ra'}
              </button>
              <button
                onClick={() => { setExitInput(''); setScanResult(null); setStatusMessage('Chưa quét thẻ'); setDemoState('normal'); setIsWaitingPayment(false); setPendingExitData(null) }}
            ...
            <div className="p-6 flex flex-col gap-4 justify-center flex-1">
                ...
                <div>
                  ...
                  <p className="font-headline font-bold text-xl text-slate-800 leading-tight">
                    {demoState === 'offline'
                      ? 'Chưa nhận dạng'
                      : scanResult
                        ? (mockDB.validateCard(scanResult.card)?.name || 'Khách vãng lai')
                        : 'Chưa xác định'}
                  </p>
                  {scanResult && (
                    <p className="text-xs text-slate-500 mt-0.5">
                      ...
                      {scanResult.userRole === 'guest' && (
                        <span className="ml-2 px-2 py-0.5 bg-amber-100 text-amber-700 rounded text-[10px] font-bold">KHÁCH VÃNG LAI</span>
                      )}
                    </p>
                  )}
                </div>
                ...
                <div className="p-3 bg-slate-50 rounded-2xl border border-slate-100 hover:shadow-sm transition-shadow">
                  ...
                  <p className="font-bold text-slate-700 text-sm">
                    {demoState === 'open' && scanResult?.exitTime
                      ? new Date(scanResult.exitTime).toLocaleTimeString('vi-VN')
                      : '---'}
                  </p>
                </div>
                <div className={`p-3 rounded-2xl border flex items-center justify-between hover:shadow-sm transition-shadow ${demoState === 'open' ? 'bg-green-50 border-green-200 text-green-900' : isWaitingPayment ? 'bg-amber-50 border-amber-200 text-amber-900 shadow-inner' : demoState === 'offline' ? 'bg-slate-50 border-slate-200 text-slate-400' : 'bg-orange-50 border-orange-200 text-orange-900 shadow-inner'}`}>
                  <div>
                    <p className="text-[10px] font-bold uppercase tracking-widest mb-1 opacity-80">
                      {demoState === 'open' && !isWaitingPayment
                        ? (scanResult?.paymentMethod === 'debt' ? 'GHI NỢ' : 'ĐÃ TT')
                        : 'TỔNG PHÍ (VNĐ)'}
                    </p>
                    <p className="font-bold text-lg leading-tight font-mono">
                      {(scanResult?.fee ?? 0).toLocaleString('vi-VN')}
                    </p>
                  </div>
              ...
              {demoState === 'open' && scanResult?.paymentMethod === 'debt' && (
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-xl text-blue-800 text-sm flex items-center gap-2">
                  ...
                  <span>Phí <strong>{(scanResult?.fee || 0).toLocaleString('vi-VN')} VNĐ</strong> đã được ghi vào công nợ thành viên.</span>
                </div>
              )}
            </div>
            ...
            <div className="p-6 border-t border-slate-100 bg-slate-50">
              <div className="grid grid-cols-2 gap-4 mb-5">
                <button
                  ...
                  disabled={demoState === 'offline' || isBarrierOpen || isWaitingPayment}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody>
                {exitLog.map((log) => {
                  ...
                  const hours = session
                    ? Math.max(1, Math.ceil((new Date(session.exitTime) - new Date(session.entryTime)) / (1000 * 60 * 60)))
                    : 0
                  ...
                  return (
                    <tr key={log.timestamp + log.card} className="border-b border-slate-50 hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="p-4">
                        <span className={`px-3 py-1 rounded-lg text-xs font-bold ${session ? paymentLabel.cls : 'bg-slate-100 text-slate-600'}`}>
                          {session ? paymentLabel.label : '---'}
                        </span>
                      </td>
                      <td className="p-4">
                ...
                {exitLog.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-8 text-center text-slate-400 text-sm">Chưa có lượt ra nào được ghi nhận</td>
                  </tr>
                )}
              </tbody>
            </table>
      ...
      {/* Guest Payment Modal — Barrier LOCKED until this is confirmed */}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-md rounded-3xl shadow-2xl overflow-hidden border-4 border-amber-400">
            ...
            <div className="bg-amber-50 px-8 pt-8 pb-6 text-center border-b border-amber-200">
              ...
              <p className="text-slate-500 text-sm">Xe Khách vãng lai — Barrier bị khóa cho đến khi thanh toán</p>
            </div>
            ...
            <div className="px-8 py-6 space-y-4">
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div className="p-3 bg-slate-50 rounded-xl">
                  ...
                  <p className="font-bold text-slate-800">{pendingExitData.session.vehicle}</p>
                </div>
                <div className="p-3 bg-slate-50 rounded-xl">
                  <p className="text-xs text-slate-400 font-bold uppercase mb-1">Thời điểm vào</p>
                  <p className="font-bold text-slate-800">{new Date(pendingExitData.session.entryTime).toLocaleTimeString('vi-VN')}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 p-3 bg-red-50 border border-red-200 rounded-xl text-red-700 text-sm">
                ...
                <span>Barrier đang bị khóa. Nhấn xác nhận sau khi nhận đủ tiền mặt hoặc quét BKPay.</span>
              </div>
            </div>
            ...
            <div className="px-8 pb-8 space-y-3">
              ...
              <button
                onClick={() => { setIsWaitingPayment(false); setPendingExitData(null); setScanResult(null); setStatusMessage('Đã hủy thanh toán') }}
        ...
        <div className="fixed inset-0 z-[100] overflow-y-auto">
          ...
          <div className="flex min-h-full items-center justify-center p-4">
            <div className="relative bg-white w-full max-w-md rounded-3xl shadow-2xl p-8 animate-in zoom-in-95 duration-200">
              ...
              <button
                onClick={() => { setShowTempCard(false); setIsBarrierOpen(true) }}
